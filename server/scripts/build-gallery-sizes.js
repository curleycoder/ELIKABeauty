...
(async () => {
  ...
  for (const file of files) {
    ...
    await sharp(input)
      ...
      .toFile(path.join(outThumb, file.replace(/\.(png|jpe?g)$/i, ".jpg")));

    // PREVIEW (900w)
    await sharp(input)
      .resize({ width: 900, withoutEnlargement: true })
      .jpeg({ quality: 80, mozjpeg: true })
      .toFile(path.join(outPreview, file.replace(/\.(png|jpe?g)$/i, ".jpg")));

    // FULL (1800w)
    await sharp(input)
      .resize({ width: 1800, withoutEnlargement: true })
      .jpeg({ quality: 85, mozjpeg: true })
      .toFile(path.join(outFull, file.replace(/\.(png|jpe?g)$/i, ".jpg")));
